...
function serveAPI(req, res){
    ...
    if(req.user){
        ...
        db.find("project", query, {}, {sort: [["name", "asc"]], limit: 10, skip: 0}, function(err, records){
            serve(req, res, {page: "/api", values:{list: records || []}});
        });
    }else{
        serve(req, res, {page: "/api", values:{list: []}});
    }
...
function serveAPIGetProject(req, res){
    var accessToken = (req.query.access_token || "").toString().trim(),
        projectId = (req.params.project || "").toString().trim();

    apiAuthorizeToken(accessToken, function(err, user){
        if(err){
...
function apiAuthorizeToken(accessToken, callback){
...
function apiActionGet(user, projectId, callback){
    ...
    db.findOne("project", {_id: new ObjectID(projectId)}, function(err, project){
        ...
        responseObject.expiration_time = project.userCertificate.expires.toUTCString();
...
function apiActionPost(user, project, callback){
    ...
    tools.generateKeys(user, user.certDays || 180, Number(project.key_size) || 1024, function(err, userCertificate, bankCertificate){
        if(err){
