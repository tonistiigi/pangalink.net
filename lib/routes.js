...
module.exports = function(app){
    ...
    app.get("/project/:project/example/render/send.php", serveRenderedExamplePayment);
    app.get("/project/:project/example/send.php", serveExamplePayment);
...
function serveExamplePayment(req, res){
    ...
    banklink.samplePayment(id, req.user.id, function(err, paymentObj, charset){
        if(err){
        ...
        var payment = {
                ...
                fields: Object.keys(paymentObj.fields).map(function(key){
                    return {key: key, value: paymentObj.fields[key]};
                })
            },
        ...
        if(!req.renderHTML){
            res.setHeader("Content-Description", "File Transfer");
            res.setHeader("content-type", "text/plain; charset=" + payment.forceCharset);
            res.setHeader("Content-Disposition", util.format("attachment; filename=\"%s\"", "send.php"));    
        }else{
            res.setHeader("content-type", "text/html; charset=" + payment.forceCharset);
        }
        
        res.render("scripts/" + (req.renderHTML?"rendered.send" : "send." + payment.bank) + ".ejs", {
            title: config.title || (config.hostname || (req && req.headers && req.headers.host) || "localhost").replace(/:\d+$/, "").toLowerCase().replace(/^./, function(s){
                return s.toUpperCase();
            }),
            proto: config.proto || "http",
...
function handleJoin(req, res){
    ...
    auth.addUser(req, req.body.username, req.body.password, {name: req.body.name, company: req.body.company, yubi: req.body.yubi, useyubi: !!(req.body.useyubi || "")}, function(err, user, options){
        if(err){
...
function handleResetLink(req, res){
    ...
    auth.initializeResetPassword(req, req.body.username, function(err, status){
        if(err){
            req.flash("error", "Andmebaasi viga");
            serve(req, res, {page: "/reset-link", values: {
                name: req.body.name || "",
...
function handleResetPassword(req, res){
    auth.resetPassword(req, req.body.username, req.body.resetToken, function(err, status, options){
        if(err){
            req.flash("error", "Andmebaasi viga");
            return res.redirect("/login");
        }
...
function serveAPIPostProject(req, res){
    ...
    apiAuthorizeToken(accessToken, req.headers.api_secret, function(err, user){
        ...
        apiActionPost(req, user, project, function(err, projectId){
            if(err){
                return apiResponse(req, res, err);
            }
            apiActionGet(user, projectId, function(err, project){
...
function apiActionGet(user, projectId, callback){
    ...
    db.findOne("project", {_id: new ObjectID(projectId)}, function(err, project){
        ...
        if(["aab", "samlink"].indexOf(banks[project.bank].type)){
            responseObject.mac_key = project.secret || undefined;
            responseObject.algo = "md5"
        }
