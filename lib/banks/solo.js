        ...
        if(typeof response == "string"){
          ...
          (this.fields[field] || "").toString().length > this.bank.fieldLength[field]){
            this.errors.push({
                ...
                error: util.format("Välja %s pikkus on %s sümbolit, lubatud on %s", this.keyPrefix + field, (this.fields[field] || "").toString().length, this.bank.fieldLength[field])
            });
        }
    }).bind(this));
};
    ...
    tools.incrTransactionCounter(function(err, transactionId){
        ...
        transaction.sign(function(err){
            ...
            var method = "GET",
                ...
                url = payment.state == "PAYED" ? payment.successTarget : (payment.state == "PAYED" ? payment.rejectTarget : payment.cancelTarget),
