...
IPizza.prototype.getReceiverAccount = function(){
    if(this.fields.VK_SERVICE == "1002"){
        return this.record.ipizzaReceiverAccount || tools.generateExampleAccountNr(this.bank.accountNrPrefix || 99, this.bank.accountNrLength || 11, this.getReceiverName());
    }else{
...
IPizzaValidator.prototype.validateFields = function(){
    ...
    if(typeof response == "string"){
        return [{field: "VK_SERVICE", value: service, error: response}];
    }
    ...
    IPizza.serviceFields[service].forEach((function(field){
        ...
        if(typeof response == "string"){
          ...
          (this.fields[field] || "").toString().length > this.bank.fieldLength[field]){
            this.errors.push({
                field: field, 
                value: (this.fields[field] || "").toString(), 
                error: util.format("Välja %s pikkus on %s sümbolit, lubatud on %s", field, (this.fields[field] || "").toString().length, this.bank.fieldLength[field])
            });
        }else if(this.bank.fieldRegex && 
          this.bank.fieldRegex[field] && 
          !(this.fields[field] || "").toString().match(new RegExp(this.bank.fieldRegex[field].re, this.bank.fieldRegex[field].flags))){
            this.errors.push({
                field: field, 
                value: (this.fields[field] || "").toString(), 
...
IPizzaValidator.prototype.validate_VK_SERVICE = function(){
    ...
    if(!IPizza.serviceFields[value]){
        return util.format("Teenuskoodi %s (\"%s\") väärtus ei ole toetatud. Kasutada saab järgmisi väärtuseid: %s", "VK_SERVICE", value, Object.keys(IPizza.serviceFields).join(", "));
    }
...
IPizzaValidator.prototype.validate_VK_ACC = function(){
    ...
    if(!value.match(/^\d{1,}$/)){
        return util.format("Saaja konto number %s (\"%s\") peab olema numbriline väärtus", "VK_ACC", value);
    }
    ...
    tools.incrTransactionCounter(function(err, transactionId){
        ...
        if(payment.state == "PAYED"){
            fields = {
                ...
                "VK_REC_ACC": paymentFields.VK_ACC || "",
                "VK_REC_NAME": paymentFields.VK_NAME || "",
